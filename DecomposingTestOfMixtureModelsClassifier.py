  ...
  for curr,pdf in enumerate(all_pdfs): 
    ...
    for i,f in enumerate(funcs):
      if isinstance(f,str):
        ...
        f.plotOn(frame,ROOT.RooFit.Name(legends[curr][i]),line_colors[i])
...
def classifierPdf():
  ...
  for k,c in enumerate(c0):
    for j,c_ in enumerate(c1):
      ...
      outputs = predict('{0}/model/{1}/{2}/adaptive_{3}_{4}.pkl'.format(dir,model_g,c1_g,k,j),traindata.reshape(traindata.shape[0],1))
  ...
  # Should I be using test data here?
  outputs = predict('{0}/model/{1}/{2}/adaptive_F0_F1.pkl'.format(dir,model_g,c1_g),traindata.reshape(traindata.shape[0],1))
  #outputs = clf.predict_proba(traindata.reshape(traindata.shape[0],1)) 
...
def fitAdaptive(use_log = False):
  ...
  def constructDensity(w,pos = None):
    ...
    nn.RegisterCallBack(lambda x: scikitlearnFunc('{0}/model/{1}/{2}/adaptive_{3}_{4}.pkl'.format(dir,model_g,c1_g,k,j),x))
    ...
    if verbose_printing == True and k <> j:
      ...
      printFrame(w,'x',[w.pdf('sigmoddist_{0}_{1}'.format(k,j)),
                w.pdf('bkgmoddist_{0}_{1}'.format(k,j))],makePlotName(full,'train',k,j,'dist'),['signal','bkg'])
  ...
  saveFig(xarray, [fullRatios],  makePlotName('comp','train',type='ratio'+post)) 
  ...
  saveFig(xarray, [pdfratios], makePlotName('full','train',type='ratio'+post))
  saveFig(xarray, [np.array(y2) - pdfratios],makePlotName('full','train',type='diff'+post))
