...
def evalC1Likelihood(test,c0,c1,dir='/afs/cern.ch/user/j/jpavezse/systematics',
  ...
  for i,cs in enumerate(csarray):
    ...
    c1s[0] = cs
    c1s = c1s/c1s.sum()
    decomposedRatios,trueRatios = test.evaluateDecomposedRatio(w,testdata,x=x, plotting=False,roc=False,c0arr=c0,c1arr=c1s,true_dist=true_dist)
    if use_log == False:
...
def fitCValues(test,c0,c1,dir='/afs/cern.ch/user/j/jpavezse/systematics',
  ...
  fil1 = open('fitting_values_c1.txt','a')
  fil2 = open('fitting_values_c1c2.txt','a')
  ...
  c1_1 = np.loadtxt('fitting_values_c1.txt')  
  c1_['true'] = c1_1[:,0]
  c1_['dec'] = c1_1[:,1]
  c1_2 = np.loadtxt('fitting_values_c1c2.txt')
  ...
  saveFig([],[c1_['true'],c1_['dec']], 
      ...
      labels=['true','composed'],x_range=[0.,0.1])
  ...
  saveFig([],[c2_values['true'],c2_values['dec']], 
      ...
      labels=['true','composed'],x_range=[0.2,0.4])
...
if __name__ == '__main__':
  ...
  #makeModelPrivateND(vars_g=vars_g,c0=c0,c1=c1,dir=dir)

  # make mixture model
  makeModelND(vars_g=vars_g,c0=c0,c1=c1,workspace=workspace_file,dir=dir)

  # make sintetic data to train the classifiers
  makeData(vars_g=vars_g,c0=c0,c1=c1,num_train=20000,num_test=30000,
